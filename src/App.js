...
class App extends Component {
    fetchMatchesList = async () => {
        const list = await fetch("http://176.119.51.91:5000/")
            .then(res => res.json())
            .then(res => {
                const matchesList = res.data.matches.map((match, i) => {
                    const player1 = Number(
                        this.state.playersList[match.home.playerId].value
                    );
                    const player2 = Number(
                        this.state.playersList[match.away.playerId].value
                    );

                    const player1ovr =
                        Number(match.home.teamAttack) +
                        Number(match.home.teamMidfield) +
                        Number(match.home.teamDefense);
                    const player2ovr =
                        Number(match.away.teamAttack) +
                        Number(match.away.teamMidfield) +
                        Number(match.away.teamDefense);

                    const gm = this.getGoalMultiplier(
                        match.home.goals,
                        match.away.goals
                    );
                    const e1 =
                        ...
                        (1 +
                            Math.pow(
                                10,
                                (player2 - player1) / 400 +
                                    (player2ovr - player1ovr) / 48
                            ));
                    const s = this.getResult(
                        match.home.goals,
                        match.away.goals
                    );
                    ...
                    playersList[match.home.playerId].goalsScored += parseInt(
                        match.home.goals
                    );
                    playersList[match.home.playerId].goalsConceded += parseInt(
                        match.away.goals
                    );

                    playersList[match.away.playerId].goalsScored += parseInt(
                        match.away.goals
                    );
                    playersList[match.away.playerId].goalsConceded += parseInt(
                        match.home.goals
                    );
            ...
            <div className="App">
                <div className="container">
                    <div className="row d-flex flex-column justify-content-center align-items-center">
                        <img
                            src="http://www.fifplay.com/img/public/fifa-18-logo-black.png"
                            alt=""
                            className="logo mt-3"
                        />
                        <h1 className="display-4 mt-2 mb-5">
                            Office Elo Soccer Rating
                        </h1>
                    </div>
                    <FormMatch
