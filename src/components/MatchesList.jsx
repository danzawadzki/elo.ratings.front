            ...
            <table className="table table-bordered table-hover table-sm table__matches">
                ...
                {(matches.status === "loaded" && matches.list.length) > 0 ? matches.list.map((match, i) => (
                    <tr key={'match-' + i}>
                        ...
                        <td className="align-middle">{playersList[match.home.playerId].name}</td>
                        <td className="align-middle">{match.home.goals} : {match.away.goals}<br/>
                            <small className="text-muted">{Math.abs(match.ratingDelta.toFixed(3))}</small>
                        </td>
                        <td className="align-middle">{playersList[match.away.playerId].name}</td>
